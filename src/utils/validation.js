export const validateProduct = (productName, categoryId, products) => {
	const existingProduct = products.find((p) => p.name.toLowerCase() === productName.toLowerCase() && p.category_id === categoryId);
...
export const validateTableNumber = (tableNumber, restaurantId, tables, excludeId = null) => {
	const existingTable = tables.find((t) => t.number === parseInt(tableNumber) && t.restaurant_id === restaurantId && t.id !== excludeId);
