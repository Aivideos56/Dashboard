      ...
      <div className="flex-1 ml-64">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            <StatCard
              title="Ümumi Gəlir"
              value={formatCurrency(stats.totalRevenue, restaurant.currency)}
              icon={DollarSign}
              color="green"
            />
            <StatCard
            ...
            <StatCard
              title="Məhsul Satışı"
              value={orders.reduce((sum, o) => sum + (o.items?.length || 0), 0)}
              icon={Package}
              color="red"
            />
          </div>
                  ...
                  {tableStats.map((stat) => (
                    <tr key={stat.tableNumber} className="hover:bg-gray-50">
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">
                        Masa {stat.tableNumber}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">{stat.orderCount}</td>
                      <td className="px-6 py-4 text-sm font-medium text-gray-900">
                        {formatCurrency(stat.totalRevenue, restaurant.currency)}
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">
                        {formatCurrency(stat.avgCheck, restaurant.currency)}
                      </td>
                    </tr>
                  ))}
                </tbody>
