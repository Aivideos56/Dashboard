...
export default function OrdersPage() {
	...
	return (
		<div className='min-h-screen bg-gray-50 flex'>
								...
								<tbody className='divide-y divide-gray-200'>
									{filteredOrders.map((order) => (
										<tr key={order.id} className='hover:bg-gray-50'>
											...
											<td className='px-6 py-4 text-sm text-gray-900'>{formatDateTime(order.completed_at)}</td>
											<td className='px-6 py-4 text-sm text-gray-900'>Masa {order.table_number}</td>
											<td className='px-6 py-4 text-sm text-gray-900'>{order.items ? order.items.length : 0}</td>
											<td className='px-6 py-4 text-sm font-medium text-gray-900'>{formatCurrency(order.total, restaurant.currency)}</td>
											<td className='px-6 py-4 text-sm text-gray-900'>{getPaymentMethodText(order.payment_method)}</td>
							...
							{filteredOrders.length === 0 && (
								<div className='text-center py-12'>
									<p className='text-gray-500'>
										{searchTerm || filterTable || filterPayment || startDate || endDate ? 'Filtrə uyğun sifariş tapılmadı' : 'Hələ sifariş yoxdur'}
									</p>
								</div>
							)}
						</div>
			...
			<OrderDetailModal isOpen={!!selectedOrder} onClose={() => setSelectedOrder(null)} order={selectedOrder} currency={restaurant?.currency} />
		</div>
	);
}
