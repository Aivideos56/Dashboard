...
export default function ModifiersPage({ restaurantId }) {
	...
	const filterModifiers = () => {
		...
		const filtered = modifiers.filter((modifier) => modifier.name.toLowerCase().includes(searchTerm.toLowerCase()));
										...
										<tbody className='bg-white divide-y divide-gray-200'>
											{filteredModifiers.map((modifier) => (
												<tr key={modifier.id} className='hover:bg-gray-50 transition'>
													...
													<td className='px-6 py-4 whitespace-nowrap'>
														<div className='text-sm text-gray-900 font-medium'>{modifier.price} ₼</div>
													</td>
													<td className='px-6 py-4 whitespace-nowrap'>
														<span
															className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
																modifier.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
															}`}
														>
													...
													<td className='px-6 py-4 whitespace-nowrap text-right text-sm font-medium'>
														<div className='flex items-center justify-end space-x-2'>
															...
															<button onClick={() => handleDeleteClick(modifier)} className='p-2 text-red-600 hover:bg-red-50 rounded-lg transition'>
					...
					{showDeleteConfirm && (
						<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4'>
							<div className='bg-white rounded-xl shadow-2xl max-w-md w-full p-6'>
								...
								<h3 className='text-lg font-bold text-gray-900 text-center mb-2'>Modifikatoru sil</h3>
								<p className='text-gray-600 text-center mb-6'>
									"{modifierToDelete?.name}" modifikatorunu silmək istədiyinizdən əminsiniz? Bu əməliyyat geri qaytarıla bilməz.
								</p>
