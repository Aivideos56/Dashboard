...
export default function HallTableModal({ isOpen, onClose, hallId, hallName, table = null, existingTables = [] }) {
  ...
  const allowedStatuses = ['empty', 'occupied', 'reserved', 'payment'];
  const allowedShapes = ['square', 'circle', 'sofa'];

  useEffect(() => {
    if (table) {
      setValue('number', table.table_number);
      ...
      const maxNumber = existingTables.length > 0
        ? Math.max(...existingTables.map((t) => t.table_number))
        : 0;
  ...
  const onSubmit = async (data) => {
    ...
    const shape = data.shape?.trim().toLowerCase();

    if (!allowedStatuses.includes(status)) {
      toast.error('Yanlış masa statusu');
      return;
    }

    if (!allowedShapes.includes(shape)) {
      toast.error('Yanlış masa forması');
      return;
    }

    if (!restaurant?.id) {
      toast.error('Restoran məlumatı tapılmadı');
      return;
    }

    if (!table) {
      const exists = existingTables.find((t) => t.table_number === tableNumber);
      if (exists) {
        toast.error(`Masa №${tableNumber} artıq mövcuddur`);
        return;
      }
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Tutum (nəfər) *</label>
            <input
              type="number"
          ...
          <div>
            ...
            <select {...register('shape')} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
          ...
          <div className="flex items-center justify-end space-x-4 pt-4">
            <button type="button" onClick={onClose} className="px-6 py-2 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-50 transition">
              Ləğv et
            </button>
            <button type="submit" disabled={loading} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition disabled:opacity-50">
